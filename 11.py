# At the biginning I did not consider the diagonal [i - ind][j + ind]
...
def oriz_prod(i, j, mat):
    prod = 1
    for ind in range(INDEX):
        try: 
            prod = prod*int(mat[i][j+ind])
        except:
            return 1
    return prod
...
for i in range(SIZE):
    for j in range(SIZE):
        oriz = oriz_prod(i, j, mat)
        ver = vertical_prod(i, j, mat)
        obl = obl_prod_right(i,j, mat)
        obl_left = obl_prod_left(i, j, mat)
        res = maximum(ver, oriz, obl)
        if res < obl_left:
            res = obl_left
        if max < res:
            max = res
